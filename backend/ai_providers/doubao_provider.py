"""
Doubao (ByteDance) Provider Implementation
字节跳动豆包AI提供商实现
"""
import asyncio
...
class DoubaoProvider(AIProvider):
    """Doubao (ByteDance) AI provider implementation"""
    ...
    MODEL_PRICING = {
        ...
        "doubao-lite-128k": (0.0008, 0.002),
        "doubao-pro-4k": (0.0008, 0.002),
        "doubao-pro-32k": (0.0008, 0.002),
        "doubao-pro-128k": (0.002, 0.008),
        "doubao-pro-256k": (0.005, 0.02),
    }
    ...
    def __init__(self, api_key: str, model: str = "doubao-lite-4k", **kwargs):
        ...
        Args:
            api_key: Doubao API key (from 火山方舟)
            model: Model name (default: doubao-lite-4k)
    ...
    def _validate_config(self) -> None:
        ...
        if not self.api_key:
            raise AIProviderError("Doubao API key is required", "doubao")
        
        if self.model not in self.get_supported_models():
            raise AIProviderError(f"Model '{self.model}' not supported", "doubao")
    ...
    async def generate_completion(
        ...
        **kwargs
        ...
        """Generate completion using Doubao"""
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    ...
                    raise AIProviderError(f"Doubao API error: {error_message}", "doubao", str(response.status_code))
            ...
            raise AIProviderError(f"Network error: {str(e)}", "doubao")
        except Exception as e:
            if isinstance(e, AIProviderError):
                raise
            raise AIProviderError(f"Unexpected error: {str(e)}", "doubao")
    ...
    def get_supported_models(self) -> List[str]:
        """Get supported Doubao models"""
        return list(self.MODEL_PRICING.keys())
    
    def estimate_cost(self, input_tokens: int, output_tokens: int) -> float:
        """Estimate cost for Doubao API usage"""
