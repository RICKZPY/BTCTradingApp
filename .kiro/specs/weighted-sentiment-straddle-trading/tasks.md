...
- [ ] 1. 创建数据模型和验证逻辑
  ...
  - [ ]* 1.2 为 WeightedNews 编写属性测试
    - **属性 6: 数据验证完整性**
    - **验证需求：8.1, 8.2, 8.3**
  
  - [ ] 1.3 实现 OptionTrade 数据类
  ...
  - [ ]* 1.4 为 OptionTrade 编写属性测试
    - **属性 7: 期权交易验证**
    - **验证需求：8.4, 8.5**
  
  - [ ] 1.5 实现 StraddleTradeResult 和 TradeRecord 数据类
    - 创建 StraddleTradeResult 包含交易结果信息
    - 创建 TradeRecord 包含完整的交易记录信息
    - _需求：4.6, 5.2_

- [ ] 2. 实现 NewsAPIClient 组件
  - [ ] 2.1 创建 NewsAPIClient 类
  ...
  - [ ]* 2.2 为 NewsAPIClient 编写单元测试
    - 使用 mock 测试 API 响应解析
    - 测试错误处理（超时、网络错误、无效响应）
    - _需求：1.4, 7.1_

- [ ] 3. 实现 NewsTracker 组件
  - [ ] 3.1 创建 NewsTracker 类和数据库初始化
  ...
  - [ ]* 3.4 为 NewsTracker 编写属性测试
    - **属性 1: 无重复交易**
    - **属性 2: 评分阈值强制执行**
    - **属性 14: 新闻历史持久化**
    - **验证需求：2.2, 2.3, 2.4, 2.5**
  
  - [ ]* 3.5 为 NewsTracker 编写单元测试
    - 测试边界情况（评分 6、7、8）
    - 测试数据库操作（插入、查询、去重）
    - _需求：2.5_

- [ ] 4. 实现 StraddleExecutor 组件
  - [ ] 4.1 创建 StraddleExecutor 类
  ...
  - [ ]* 4.5 为 execute_straddle 编写属性测试
    - **属性 3: 跨式完整性**
    - **属性 15: 跨式成本计算**
    - **验证需求：3.4, 4.5**
  
  - [ ]* 4.6 为 StraddleExecutor 编写单元测试
    - 测试部分执行失败的回滚逻辑
    - 测试找不到合适期权的错误处理
    - _需求：3.5, 7.2, 7.3_

- [ ] 5. 实现 TradeLogger 组件
  - [ ] 5.1 创建 TradeLogger 类和数据库初始化
  ...
  - [ ]* 5.4 为 TradeLogger 编写属性测试
    - **属性 4: 交易-新闻关联**
    - **属性 8: 交易记录持久化往返**
    - **属性 9: 查询结果排序**
    - **验证需求：5.2, 5.6, 9.5**
  
  - [ ]* 5.5 为 TradeLogger 编写单元测试
    - 测试数据库事务和错误处理
    - 测试查询边界情况（空结果、日期范围）
    - _需求：5.6, 9.4_

- [ ] 6. 实现 WeightedSentimentService 主服务
  - [ ] 6.1 创建 WeightedSentimentService 类
  ...
  - [ ]* 6.5 为 WeightedSentimentService 编写属性测试
    - **属性 5: 每小时执行频率**
    - **验证需求：6.2, 6.4**
