  ...
  loadData();
    ...
    async def handle_iv_chart(self, request):
        ...
        html = (
            ...
            "      {label:'新闻事件',type:'scatter',data:_newsEvents.map(e=>{const c=dvolPoints.length?dvolPoints.reduce((a,b)=>Math.abs(b.x-e.ts)<Math.abs(a.x-e.ts)?b:a,dvolPoints[0]):{y:0};return{x:e.ts,y:c.y,s:e.score,t:e.content,u:e.news_id};}).filter(p=>p.y>0&&(_filterMin===0||p.s>=_filterMin&&(_filterMin===10?p.s===10:_filterMin===8?p.s>=8&&p.s<10:p.s===7))),pointBackgroundColor:function(c){const s=c.raw&&c.raw.s||0;return s>=10?'rgba(255,59,48,0.9)':s>=8?'rgba(255,149,0,0.9)':'rgba(255,204,0,0.85)';},pointRadius:function(c){const s=c.raw&&c.raw.s||0;return s>=10?9:s>=8?7:5;},pointHoverRadius:12,pointStyle:'triangle',showLine:false,yAxisID:'y',order:0},\n"
